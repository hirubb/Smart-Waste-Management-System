...
import { Container, Row, Col, Card, Table, Badge, Button, Form, InputGroup, Modal, Navbar, Nav, Dropdown, ListGroup } from "react-bootstrap";
import { FaBell, FaSearch, FaSync, FaUserCircle, FaExclamationTriangle, FaInfoCircle, FaCheckCircle, FaTimes } from "react-icons/fa";
...
const AssignedRoutes = () => {
  ...
  // Fetch alerts from API - filtered for Waste Collector
  useEffect(() => {
    fetchAlerts();
  }, []);
  ...
  const fetchAlerts = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        const formattedAlerts = collectorAlerts.map(alert => ({
          ...alert,
  ...
  const generateNotificationsFromAlerts = (alertsList) => {
    ...
    alertsList.forEach((alert) => {
      ...
      notifs.push({
        ...
        time: timeAgo,
        type: notifType,
        read: alert.status === 'Resolved' // Mark as read if already resolved
      });
    });
    ...
    <>
      ...
      <Navbar bg="white" expand="lg" className="shadow-sm px-4 py-3" style={{ borderBottom: '2px solid #e0e0e0' }}>
        <Container fluid>
          ...
          <Navbar.Toggle aria-controls="collector-navbar" />
          
          <Navbar.Collapse id="collector-navbar">
            ...
            <Nav className="mx-auto">
              ...
              <Nav.Link as={Link} to="/assigned-routes" className="mx-2 px-3 py-2" style={{ 
                border: '1px solid #d0d0d0', 
                borderRadius: '4px',
                color: '#333',
                fontWeight: '500',
              ...
              <Nav.Link as={Link} to="/collection-status" className="mx-2 px-3 py-2" style={{ 
                border: '1px solid #d0d0d0', 
                borderRadius: '4px',
                color: '#333',
                fontWeight: '500'
              ...
              <Nav.Link as={Link} to="/route-history" className="mx-2 px-3 py-2" style={{ 
                border: '1px solid #d0d0d0', 
                borderRadius: '4px',
                color: '#333',
                fontWeight: '500'
      ...
      <Container fluid className="py-4" style={{ backgroundColor: '#f8f9fa', minHeight: '100vh' }}>
        ...
        <div className="mb-3">
          ...
          <span className="fw-bold">Assigned Routes</span>
        </div>
      ...
      <div className="mb-4">
        ...
        <p className="text-muted">
          View and manage waste collection routes assigned to you. Keep track of bin alerts and collection schedules.
        </p>
      </div>
            ...
            <div className="text-center py-5">
              ...
              <p className="text-muted">
                {searchTerm || selectedSeverity !== 'all' || selectedStatus !== 'all' || selectedType !== 'all'
                  ...
                  : 'You currently have no assigned collection routes. New assignments will appear here.'}
              </p>
            </div>
          ) : (
